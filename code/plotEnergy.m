...
if plotEnergy
    ...
    p_vel_eff = hggroup('displayname', 'efficiency(and error)');
    errorbar(vel_bins_mid(ok), slope(ok), 2*slope_e(ok), 'DisplayName', 'calculated efficiency-velocity', 'Parent',p_vel_eff);
    ...
    c = [c;axs.Children];
    c = c([5 6 1 4 2 3]);
    dnames = arrayfun(@(x) x.DisplayName, c, 'un', 0);
    legend(c, dnames);
end
